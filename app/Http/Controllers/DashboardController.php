...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user_type == 'customer') {
            ...
            $data['total_deposit_amount'] = Transaction::where('type', 'Deposit')->sum('amount');

            // Total withdrawal amounts
            $data['total_withdrawal_amount'] = Transaction::where('type', 'Withdraw')->sum('amount');

            // Total revenue
            $data['balance'] = Transaction::where('type', 'Deposit')->sum('amount') - Transaction::where('type', 'Withdraw')->sum('amount');

            // Sum total monthly charges from Transaction table
            $data['total_monthly_charges'] = Transaction::whereMonth('trans_date', date('m'))
