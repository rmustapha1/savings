...
class ReportController extends Controller
{
		...
		if ($request->isMethod('get')) {
			...
			$data['report_data'] = Transaction::select('transactions.*')
				...
				->when($collector_id, function ($query, $collector_id) {
					return $query->whereHas('collector', function ($query) use ($collector_id) {
						return $query->where('collector_id', $collector_id);
					});
				})
	...
	public function revenue_report(Request $request)
	{
		if ($request->isMethod('get')) {
			...
			$transaction_revenue = Transaction::selectRaw("CONCAT('Revenue from ', type) as type, sum(charge) as amount")
				->whereRaw("YEAR(trans_date) = '$year'")
				->where('charge', '>', 0)
				->where('status', 2)
			...
			if ($month != 0) {
				$transaction_revenue->whereRaw("MONTH(trans_date) = '$month'");
			}

			$maintainaince_fee = Transaction::selectRaw("CONCAT('Revenue from ', type) as type, sum(amount) as amount")
				->whereRaw("YEAR(trans_date) = '$year'")
			...
			if ($month != 0) {
				$maintainaince_fee->whereRaw("MONTH(trans_date) = '$month'");
			}
			...
			$others_fee = Transaction::join('transaction_categories', function ($join) {
				...
				->selectRaw("CONCAT('Revenue from ', type) as type, sum(amount) as amount")
				->whereRaw("YEAR(trans_date) = '$year'")
				->where('dr_cr', 'dr')
			...
			if ($month != 0) {
				$others_fee->whereRaw("MONTH(trans_date) = '$month'");
			}

			$data['report_data'] = $transaction_revenue
