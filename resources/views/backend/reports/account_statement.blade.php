		...
		<div class="card">
			...
			<div class="card-body">
				...
				<div class="report-params">
					<form class="validate" method="post" action="{{ route('reports.account_statement') }}"
						autocomplete="off">
						<div class="row">
							{{ csrf_field() }}
							...
							<div class="col-xl-3 col-lg-4">
								<div class="form-group">
									...
									<input type="text" class="form-control" name="account_number"
										value="{{ isset($account_number) ? $account_number : old('account_number') }}"
										required>
								</div>
							</div>
			...
			<table class="table table-bordered report-table">
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-right">
							{{ decimalPlace($transaction->debit, currency($account->savings_type->currency->name)) }}
						</td>
						<td class="text-right">
							{{ decimalPlace($transaction->credit, currency($account->savings_type->currency->name)) }}
						</td>
						<td class="text-right">
							{{ decimalPlace($transaction->balance, currency($account->savings_type->currency->name)) }}
						</td>
					</tr>
					@endforeach
					@endif
