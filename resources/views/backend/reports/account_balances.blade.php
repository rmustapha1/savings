		...
		<div class="card">
			...
			<div class="card-body">
				...
				<div class="report-params">
					<form class="validate" method="post" action="{{ route('reports.account_balances') }}"
						autocomplete="off">
						<div class="row">
							{{ csrf_field() }}
							...
							<div class="col-xl-3 col-lg-4">
								<div class="form-group">
									...
									<input type="text" class="form-control" name="member_no"
										value="{{ isset($member_no) ? $member_no : old('member_no') }}" required>
								</div>
							</div>
			...
			<table class="table table-bordered report-table">
				...
				<tbody>
					...
					<tr>
						<td>{{ $account->account_number }} - {{ $account->savings_type->name }}
							({{ $account->savings_type->currency->name }})</td>
						<td class="text-right">
